...
# 需要输出user特征    userfeat.csv
user_feat_name = ["brand_buy_per_cart", "buy_categorys_count", "buy_brands_count", "brand_buy_per_click", "favor_categorys_count", "buy_days_count", "buy_counts", "click_categorys_count", "active_days_count_per_days", "active_days_count", "cart_brands_count", "favor_brands_count", "brand_buy_per_favor", "click_brands_count", "cart_counts", "buy_counts_per_buy_days_count", "buy_days_count_per_active_days_count", "buy_per_favor", "buy_per_cart", "cart_categorys_count", "click_counts", "buy_per_click", "favor_counts"]
# 需要输出的user_all特征       userfeat_all.csv
useful_feat_name = ["buy_counts", "cart_counts", "favor_counts", "click_counts", "buy_brands_count"]
...
def initial_tmp_features(tmp_features):
    ...
    for featurenames in basic_feature_behaviour.values():
        tmp_features[featurenames[0]] = 0.0    # xxx_counts # 销量 # 购物车量 # 收藏量 # 点击量
...
def initial_user_features(other_features):
    ...
    for featurenames in other_feature_behaviour.values():
        other_features[featurenames[0]] = 0.0    # xxx_brands_count
        other_features[featurenames[1]] = 0.0    # xxx_categorys_count

    other_features['active_days_count'] = 0.0
    other_features['buy_days_count'] = 0.0

    # 购买转化率
    other_features['buy_per_cart'] = 0.0
    other_features['buy_per_favor'] = 0.0
    other_features['buy_per_click'] = 0.0
    other_features['brand_buy_per_cart'] = 0.0
    other_features['brand_buy_per_favor'] = 0.0
    other_features['brand_buy_per_click'] = 0.0

    # 比值特征
    other_features['buy_days_count_per_active_days_count'] = 0.0
    other_features['buy_counts_per_buy_days_count'] = 0.0
    other_features['active_days_count_per_days'] = 0.0
...
def extract_user_features(train_file_path, begin_date, end_date):
    ...
    print "\n" + begin_date + "---" + end_date + "extracting user features..."

    generate_sortedfile(train_file_path, "temp/sorted_by_user-" + train_file_path.split('/')[-1], 0)
    train_file = open("temp/sorted_by_user-" + train_file_path.split('/')[-1])

    users_feat_file_path = "./feature/" + begin_date + "-" + end_date + "-userfeat.csv"
    user_features_file = open(users_feat_file_path, 'w')
    users_all_feat_file_path = "./feature/" + begin_date + "-" + end_date + "-userfeat_all.csv"
    users_all_feat_file = open(users_all_feat_file_path, 'w')
    ...
    user_features_file.write("user_id" + "," + delimiter.join(user_feat_name) + ",\n")  # 输出标题
    users_all_feat_file.write("user_id" + "," + delimiter.join(useful_feat_name) + ",\n")    # 输出标题
    ...
    for line in train_file:
        ...
        if not user_id == pre_user_id:
            ...
            # users_all_feat_file.write(pre_user_id + "," + get_user_features_str(dict(tmp_features, **other_features)) + "\n")  # 输出当前user_allfeatures
            user_features, useful_features = merge_features_by_dict(tmp_features, other_features, user_feat_name, useful_feat_name)
            user_features_file.write(pre_user_id + "," + get_features_str_by_dict(user_features, user_feat_name) + "\n")  # 输出当前user_features
            users_all_feat_file.write(pre_user_id + "," + get_features_str_by_dict(useful_features, useful_feat_name) + "\n")    # 输出当前usefule_features
    ...
    users_all_feat_file.close()
    train_file.close()

    print "extract" + begin_date + "---" + begin_date + "user features completed"
    return users_feat_file_path
...
def get_other_basic_user_features(tmp_features):
    ...
    #比值特征
    other_features['active_days_count_per_days'] = float(other_features['active_days_count']) / (days)
...
# extract_user_features(train_file_path, "2014-12-9", "2014-12-18")
